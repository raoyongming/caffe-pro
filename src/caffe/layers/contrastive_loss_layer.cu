...
void ContrastiveLossLayer<Dtype>::Forward_gpu(
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    ...
    else {if(static_cast<int>(bottom[2]->cpu_data()[i]) == 0) {  // dissimilar pairs
      if (legacy_version) {
  ...
  if(static_cast<Dtype>(bottom[0]->num()) - counttt>0)
    loss = loss / (static_cast<Dtype>(bottom[0]->num()) - counttt) / Dtype(2);
  else
    loss = Dtype(0);
...
__global__ void CLLBackward(const int count, const int channels,
  ...
  CUDA_KERNEL_LOOP(i, count) {
    ...
    if(static_cast<int>(y[n])==0) {  // dissimilar pairs
      Dtype mdist(0.0);
      Dtype beta(0.0);
      if (legacy_version) {
